...
void *handle_clnt(int client_sock) {
	int i; int recv = 0, str_len = 0; int readcnt = 0; // read count 
	...
	recv += str_len; strcpy(method, strtok(msg, " ")); 
...
{ 
	...
	write(client_sock, contenttype, strlen(contenttype));
	write(client_sock, end, strlen(end)); write(client_sock, html, strlen(html)); 
	fflush(fdopen(client_sock, "w"));
}
